...
function buildMetadata(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    var filteredMetadata = metadata.filter((item) => item.id === parseInt(sample));   
    ...
    Object.entries(filteredMetadata[0]).forEach(([key, value]) => {
        ...
        // Capitalize every letter of each key
...
function buildCharts(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    var filteredSample = samples.filter((item) => item.id === sample);
